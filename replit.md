...
- **Publish & Share (Step 5 — `PublishStep.tsx`):** Premium publish step with status badge (Draft/Ready/Published), readiness checklist (pricing validated, lead form configured, test passed), explicit publish gating, hosted page URL with copy/open buttons, 3 embed tabs (Script/Iframe/Button Trigger) with copy-to-clipboard, collapsible installation guides (WordPress/Shopify/Custom HTML), health indicators (pricing, lead capture, anti-spam, version), republish banner for post-publish changes, and "Create Another Calculator" reset. Publish data stored in `calculator_settings.publish` with version/status/slug/published_at/embed_id/last_modified. Slug uniqueness checked via `GET /api/calculators/check-slug`.
