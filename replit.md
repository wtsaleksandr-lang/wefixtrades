...
- **AI Fallback**: Only triggers when charge_method === 'not_sure' OR mapper fails validation. Uses async job-based generation.
- **AI Agent** (`server/aiPricingAgent.ts`): Constrained composer — strict system prompt, few-shot examples, simple numeric derivation from sample quotes, multi-layer validation (schema + confidence policy + unknown field rejection). Allowed families/operations only.
- **Async Draft Flow**: POST /api/ai/pricing-config-draft → job_id, GET polls for completion. In-memory job store with 5-min TTL. Client polls with ref-based interval, non-blocking banner in Step 3.
- **Sample Quotes**: Optional 3-row mini-form in Step 2 (small/typical/big jobs with qty + price + notes). Enables derivation-first pricing (rate = price_diff / qty_diff) before AI fallback.
- **Confidence Policy**: score < 60 OR missing sample quotes → price_range_only or call_for_quote_only with needs_human_review=true
- **Audit Logging**: Source (derivation/ai/fallback), derivation result, timestamp stored in calculator_settings.pricing_audit
- **Data Schema**: `pricingIntakeSchema` (version: 1, stage1: CustomTradeData, stage2: Stage2Data, optional sample_quotes) in shared/schema.ts

## Recent Changes
- Feb 24 2026: AI Pricing Agent + Async Draft Flow — Built server/aiPricingAgent.ts as constrained composer with strict system prompt, few-shot examples, derivation-first logic from sample quotes, and multi-layer validation. Added async job-based endpoints (POST/GET /api/ai/pricing-config-draft) with in-memory job store and 5-min TTL. Client gets optional 3-row sample quotes mini-form in Step 2, ref-based polling with stale closure protection, non-blocking banner in Step 3. Audit logging (source, derivation result, timestamp) persisted to calculator_settings.pricing_audit. Confidence policy enforces conservative fallback when score < 60 or no samples.
