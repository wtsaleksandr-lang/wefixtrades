...
- **Theming:** Marketing site uses a muted, warm Optimal-inspired design system with strict tokens: `bg: #FFFFFF`, `surface: #F7F7F6`, `text/heading: #111111`, `body: #444444`, `muted: #6B6B6B`, `sage: #4A7C6F` (muted desaturated sage accent), `sageDark: #3B6358`, `sageLight: #5E9485`, `border: #E5E5E3`, `bgGray: #F7F7F6`. Headlines are near-black (#111111, not green). Accent sage used on buttons, badges, links. Header is flat white bar (56px fixed height, border on scroll). Hero uses warm gradient bg (`#F7F7F6 → #FFFFFF`), pill-shaped CTA buttons (borderRadius 9999). Footer uses `#1A1A1A` charcoal. Builder UI uses platform theme, customer-facing calculators use `WidgetTheme` (default sky blue, isolated to `.widget-scope`).
