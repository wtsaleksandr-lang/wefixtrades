 ...
 *   - axe-core violations (count + rule ids)
 *   - full-page screenshot (PNG)
 *
 * Results are written to:
 *   docs/operations/visual-audit-screenshots/audit-results.json
 *   docs/operations/visual-audit-screenshots/<route>-<viewport>.png
 *
 * Run with the included audit-top4.config.ts (no webServer; hits prod).
 */
...
const ROUTES = [
  ...
  { name: 'products-tradeline', path: '/products/tradeline' },
  ...
  for (const viewport of VIEWPORTS) {
    test(`${route.name} @ ${viewport.name}`, async ({ browser }) => {
      ...
      results.push({
        ...
        screenshot: `docs/operations/visual-audit-screenshots/${screenshotName}`,
      });
