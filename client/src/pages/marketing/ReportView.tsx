...
export default function ReportView({ report, business, reportId, liveSpeedData, speedLoading, liveWebsiteAIAnalysis, liveWebsiteScreenshot, liveWebsiteQualityCheckScore, narrativeUpgrading = false, unlocked = true, onUnlock }: {
  ...
  // Safety (false-advertising): pre-launch reviews are composites/anonymized.
  // Rendering a Google / Facebook / Trustpilot logo implies a verified
  // third-party review we can't substantiate, so all entries get a single
  // neutral "verified" check mark regardless of the (retained) platform tag.
  const PlatformIcon = (_props: { platform: string }) => (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" fill="#10B981"/>
    </svg>
  );
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 11, color: GREY, fontWeight: 500 }}>
                        {/* Safety (false-advertising): pre-launch composite
                            reviews must not claim a specific third-party
                            platform. All entries render as "Verified customer". */}
                        Verified customer
                      </span>
                    </div>
