...
import {
  ArrowLeft, Mail, Phone, Globe, MapPin, Plus,
} from "lucide-react";
                  ...
                  {services?.length === 0 ? (
                    ...
                    services?.map((s) => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell className="hidden md:table-cell text-xs text-gray-500 capitalize">{s.fulfillment_mode || "-"}</TableCell>
                        <TableCell className="hidden md:table-cell text-sm">{fmt(s.price_cents)}{s.billing_period === "monthly" ? "/mo" : ""}</TableCell>
                        <TableCell className="hidden md:table-cell text-sm">{fmt(s.cost_cents)}{s.billing_period === "monthly" ? "/mo" : ""}</TableCell>
          ...
          <TabsContent value="fulfillment" className="mt-4">
            <Card>
              <div className="p-4 border-b border-gray-100">
                <h3 className="text-sm font-semibold text-gray-900">Fulfillment Tasks ({fulfillment?.length ?? 0})</h3>
              </div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {fulfillment?.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-6 text-gray-500 text-sm">No fulfillment tasks.</TableCell>
                    </TableRow>
                  ) : (
                    fulfillment?.map((t) => (
                      <TableRow key={t.id}>
                        ...
                        <TableCell className="hidden md:table-cell text-sm">{fmt(t.cost_cents)}</TableCell>
                        <TableCell className="hidden md:table-cell text-sm">{fmtDate(t.due_at)}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
                  ...
                  {payments?.length === 0 ? (
                    ...
                    payments?.map((p) => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell className="hidden md:table-cell text-sm text-gray-500">{p.description || "-"}</TableCell>
                        <TableCell className="hidden md:table-cell text-sm">{fmtDate(p.paid_at || p.created_at)}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
