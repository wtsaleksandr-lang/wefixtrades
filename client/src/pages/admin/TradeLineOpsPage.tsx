...
interface CallRow { id: number; client_service_id: number; caller_number: string | null; duration_seconds: number | null; outcome: string; summary: string | null; ended_at: string | null; created_at: string | null; transcript_json: any; recording_url: string | null; mirrored_object_key?: string | null; mirrored_at?: string | null; business_name: string; client_id: number; }
...
function VariantBadge({ variant }: { variant: string }) { const s: Record<string,string> = { call_backup: "bg-blue-50 text-blue-700", chat: "bg-brand-blue-50 text-brand-blue-700", complete: "bg-emerald-50 text-emerald-700" }; const l: Record<string,string> = { call_backup: "Call Backup", chat: "Chat", complete: "Complete" }; return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${s[variant] || "bg-gray-100 text-gray-600"}`}>{l[variant] || variant}</span>; }
...
function CallsTab() {
  ...
  return (
    <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b bg-gray-50/80"><th className="text-left px-4 py-3 font-medium text-gray-600">Client</th><th className="text-left px-4 py-3 font-medium text-gray-600">Caller</th><th className="text-left px-4 py-3 font-medium text-gray-600">Time</th><th className="text-left px-4 py-3 font-medium text-gray-600">Duration</th><th className="text-left px-4 py-3 font-medium text-gray-600">Outcome</th><th className="text-left px-4 py-3 font-medium text-gray-600">Summary</th></tr></thead><tbody className="divide-y divide-gray-100">{calls.map(call => (<tr key={call.id} className="hover:bg-gray-50/50"><td className="px-4 py-3" colSpan={6}><button type="button" className="w-full text-left" onClick={() => setExpandedCallId(expandedCallId === call.id ? null : call.id)}><div className="grid grid-cols-6 gap-2 items-center"><span className="font-medium text-gray-900 truncate">{call.business_name}</span><span className="text-gray-600 truncate">{call.caller_number || "Unknown"}</span><span className="text-gray-500">{relativeTime(call.created_at)}</span><span className="text-gray-700 font-mono">{call.duration_seconds ? `${Math.floor(call.duration_seconds/60)}:${String(call.duration_seconds%60).padStart(2,"0")}` : "-"}</span><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium w-fit capitalize ${call.outcome==="answered"?"bg-emerald-50 text-emerald-700":call.outcome==="failed"?"bg-red-100 text-red-800":"bg-gray-100 text-gray-600"}`}>{call.outcome}</span><div className="flex items-center gap-2"><span className="text-gray-500 text-xs truncate">{call.summary||"-"}</span><ChevronDown className={`w-3.5 h-3.5 text-gray-400 shrink-0 transition-transform ${expandedCallId===call.id?"rotate-180":""}`}/></div></div></button>{expandedCallId===call.id&&(<div className="mt-3 space-y-3 border-t border-gray-100 pt-3">{call.summary&&<div><p className="text-[10px] font-medium text-gray-500 uppercase mb-1">Summary</p><p className="text-xs text-gray-700">{call.summary}</p></div>}{callDetail?.call?.transcript_json?.lead_data&&(<div><p className="text-[10px] font-medium text-gray-500 uppercase mb-1">Extracted Lead</p><div className="grid grid-cols-4 gap-1.5 text-xs">{callDetail.call.transcript_json.lead_data.caller_name&&<div><span className="text-gray-400">Name:</span> {callDetail.call.transcript_json.lead_data.caller_name}</div>}{callDetail.call.transcript_json.lead_data.job_type&&<div><span className="text-gray-400">Job:</span> {callDetail.call.transcript_json.lead_data.job_type}</div>}</div></div>)}{callDetail?.call?.recording_url&&<div><p className="text-[10px] font-medium text-gray-500 uppercase mb-1 flex items-center gap-1.5">Recording{callDetail.call.mirrored_at&&<span className="px-1.5 py-px rounded-sm bg-emerald-50 text-emerald-700 text-[9px] font-medium normal-case" title={`Mirrored to durable storage on ${callDetail.call.mirrored_at}`}>mirrored</span>}</p><audio controls className="w-full h-8" src={callDetail.call.recording_url} onError={(e)=>{const el=e.currentTarget;const note=el.nextElementSibling as HTMLElement|null;if(note)note.style.display="block";}}/><p className="text-[10px] text-amber-700 mt-1 hidden">{callDetail.call.mirrored_object_key?`Original Vapi recording expired (likely past 30-day retention). Durable copy mirrored at object key: ${callDetail.call.mirrored_object_key}`:"Original Vapi recording expired (likely past 30-day retention) and no mirror is available."}</p></div>}{callDetail?.call?.transcript_json?.text&&<div><p className="text-[10px] font-medium text-gray-500 uppercase mb-1">Transcript</p><div className="bg-gray-50 rounded-lg p-3 max-h-48 overflow-y-auto text-xs text-gray-700 whitespace-pre-wrap">{callDetail.call.transcript_json.text}</div></div>}<div><p className="text-[10px] font-medium text-gray-500 uppercase mb-1">Rate this AI handling</p><AiResponseRating responseId={`tradeline-call-${call.id}`} surface="tradeline_voice" clientId={call.client_id ?? null} /></div></div>)}</td></tr>))}</tbody></table></div>{callsData&&callsData.total>calls.length&&<div className="px-4 py-3 border-t border-gray-100 text-xs text-gray-500 text-center">Showing {calls.length} of {callsData.total} calls</div>}</Card>
  );
}
