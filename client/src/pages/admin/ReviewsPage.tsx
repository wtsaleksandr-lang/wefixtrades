...
export default function ReviewsPage() {
  ...
  const [bulkResult, setBulkResult] = useState<{
    action: string; drafted?: number; posted?: number; skipped: number; failed: number;
  ...
  const bulkDraftMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      setBulkResult(null); setShowBulkDetails(false);
      const res = await apiRequest("POST", "/api/admin/crm/monitored-reviews/bulk-draft", { ids });
      return res.json();
    },
    onSuccess: (data) => {
      setBulkResult({ action: "draft", drafted: data.drafted, skipped: data.skipped, failed: data.failed, details: data.details || [] });
      clearSelection();
      queryClient.invalidateQueries({ queryKey: ["/api/admin/crm/monitored-reviews"] });
    },
  ...
  const bulkPostMutation = useMutation({
    ...
    onSuccess: (data) => {
      setBulkResult({ action: "post", posted: data.posted, skipped: data.skipped, failed: data.failed, details: data.details || [] });
      clearSelection();
      queryClient.invalidateQueries({ queryKey: ["/api/admin/crm/monitored-reviews"] });
    },
