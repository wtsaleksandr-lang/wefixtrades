/**
 * /portal/adflow/notifications — Wave 30.
 *
 * Customer notification preferences for AdFlow events. Event × channel
 * matrix persisted in clients.metadata.adflow_notifications.
 *
...
export default function AdFlowNotificationSettings() {
  usePageTitle("AdFlow notifications");
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const { data, isLoading } = useQuery<SettingsResponse>({
    queryKey: ["/api/portal/adflow/notification-settings"],
    queryFn: async () => {
      ...
      <div className="flex flex-col gap-4 p-4 md:p-6">
        <div className="flex flex-col">
          <h1 className="flex items-center gap-2 text-xl font-semibold text-foreground md:text-2xl">
            ...
            AdFlow notifications
          </h1>
          <p className="text-sm text-muted-foreground">
            Pick which AdFlow events alert you, and on which channels.
          </p>
        </div>
        ...
        <Button variant="ghost" size="sm" asChild>
          <Link href="/portal/adflow/dashboard">Back to dashboard</Link>
        </Button>
      </div>
    </PortalLayout>
  );
