...
export default function PortalSocialSync() {
    ...
    return (
      <PortalLayout>
        <div className="max-w-3xl text-center py-16">
          <Share2 className="w-8 h-8 text-muted-foreground/50 mx-auto mb-3" />
          <p className="text-muted-foreground font-medium">You haven't set up SocialSync yet</p>
          <p className="text-sm text-muted-foreground/70 mt-1 mb-4">Tell us about your business and we'll start creating content for you.</p>
  ...
  const overviewBody = (
    <div className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{isAutoPostPaused ? "Paused" : "Active"}</p>
              </div>
            </div>
            <Switch
        ...
        {/* Connected Platforms */}
        <Card className="p-5">
          <h3 className="text-sm font-semibold text-foreground mb-3">Your Platforms</h3>
          <div className="space-y-2">
            {data.platforms.map(p => (
              <div key={p.platform} className={`flex items-center justify-between p-3 rounded-lg border ${p.connected ? "border-emerald-200 bg-emerald-50/50" : "border-border"}`}>
        ...
        {pending?.posts && pending.posts.length > 0 && (
          <Card className="p-5 border-amber-200 bg-amber-50/30">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="text-sm font-semibold text-foreground">Posts awaiting your review</h3>
                ...
                return (
                  <div key={p.id} className="bg-card rounded-lg border border-border p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[11px] font-medium text-muted-foreground capitalize">{p.platform}</span>
                    ...
                    {isEditing ? (
                      <textarea
                        className="w-full text-sm text-foreground border border-border rounded-md p-2 font-sans resize-y min-h-[100px] focus:outline-none focus:ring-2 focus:ring-[#0d3cfc]/20"
                        value={editText}
                        onChange={(e) => setEditText(e.target.value)}
                        maxLength={3000}
                      />
                    ) : (
                      <p className="text-sm text-foreground whitespace-pre-wrap">{p.post_text}</p>
                    )}
        ...
        {data.recent_posts.length > 0 && (
          <Card className="p-5">
            <h3 className="text-sm font-semibold text-foreground mb-3">Recent Content We Published</h3>
            <div className="space-y-3">
              {data.recent_posts.map(post => (
                <div key={post.id} className="flex gap-3 pb-3 border-b border-border last:border-0 last:pb-0 cursor-pointer hover:bg-muted/50 rounded-lg -mx-1 px-1 transition-colors" onClick={() => setSelectedPost(post)}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[10px] font-medium text-muted-foreground capitalize">{post.platform}</span>
                      {post.published_at && <span className="text-[10px] text-muted-foreground/70">{post.published_at}</span>}
                    </div>
        ...
        {data.upcoming_posts.length === 0 && data.recent_posts.length > 0 && (
          <Card className="p-4 text-center">
            <p className="text-xs text-muted-foreground/70">All scheduled posts have been published. New content will be generated soon.</p>
          </Card>
        )}
        {data.upcoming_posts.length > 0 && (
          <Card className="p-5">
            <h3 className="text-sm font-semibold text-foreground mb-3">Coming Up</h3>
            <div className="space-y-2">
              {data.upcoming_posts.map((post, i) => {
                ...
                return (
                  <div key={post.id}>
                    {showDate && (
                      <p className="text-[10px] font-medium text-muted-foreground/70 uppercase tracking-wider mt-2 mb-1 first:mt-0">{post.scheduled_date}</p>
                    )}
                    <div className="flex gap-3 py-2 border-b border-gray-50 last:border-0 cursor-pointer hover:bg-muted/50 rounded-lg -mx-1 px-1 transition-colors" onClick={() => setSelectedPost(post)}>
                      {post.has_image && post.image_url ? (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="text-[10px] font-medium text-muted-foreground">{post.platform}</span>
                          <span className="text-[10px] text-muted-foreground/70">{post.scheduled_for}</span>
                        </div>
        ...
        {s.autopilot && (
          <p className="text-[10px] text-muted-foreground/70 text-center pb-4">
    ...
    <PortalLayout>
      <div>
        <PortalProductPageShell
          ...
          setupBanner={
            <Card className={`p-5 ${data.status === "active" ? "bg-[#EEF3FF] border-[#0d3cfc]/10" : "bg-muted/50"}`}>
              <div className="flex items-start gap-3">
                {data.status === "active" ? (
                  <CheckCircle className="w-5 h-5 text-[#0d3cfc] mt-0.5 flex-shrink-0" />
                ) : (
                  <Clock className="w-5 h-5 text-muted-foreground/70 mt-0.5 flex-shrink-0" />
                )}
                <div>
                  <p className={`text-sm font-semibold ${data.status === "active" ? "text-[#0d3cfc]" : "text-foreground"}`}>{msg.headline}</p>
      ...
      {selectedPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setSelectedPost(null)}>
          <div className="bg-card rounded-xl border border-border shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-muted-foreground">{selectedPost.platform}</span>
                {selectedPost.published_at && <span className="text-xs text-muted-foreground/70">{selectedPost.published_at}</span>}
                {"scheduled_for" in selectedPost && (selectedPost as any).scheduled_for && (
                  <span className="text-xs text-muted-foreground/70">Scheduled: {(selectedPost as any).scheduled_for}</span>
                )}
              </div>
              <button onClick={() => setSelectedPost(null)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground/70">
                <X className="w-4 h-4" />
              </button>
            </div>
