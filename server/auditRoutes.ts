...
/* ─── Report cache versioning (Fix 3 — cache-bust on deploy) ──────────────────
 * The /generate endpoint serves a cached report (24h TTL) keyed only on
 * business_place_id + created_at. After a deploy that changes audit LOGIC, the
 * 24h cache happily served reports generated by the OLD code — so users (and the
 * owner) saw stale reports post-publish. We now stamp a cache-version token into
 * each saved report's audit_data and require it to match the CURRENT token on
 * read. A new deploy naturally invalidates old cached reports without a DB
 * migration or a manual purge.
 *
 * The token combines:
 *   • CACHE_VERSION — a manual constant. BUMP IT whenever audit logic changes in
 *     a way that should invalidate cached reports (this PR bumps it).
 *   • the runtime build SHA (GIT_SHA / deployment id) when available — so even
 *     forgetting to bump CACHE_VERSION, a fresh deploy still busts the cache.
 *
 * forceRefresh still bypasses the cache entirely (unchanged). */
const CACHE_VERSION = "2026-06-15.1";

/** The effective report cache version for this running build. Manual
 *  CACHE_VERSION + the deploy's build SHA (when present) so a deploy busts the
 *  cache even if CACHE_VERSION wasn't bumped. Computed once per process. */
const REPORT_CACHE_VERSION: string = (() => {
  const sha =
    process.env.GIT_SHA ??
    process.env.REPL_DEPLOYMENT_ID ??
    process.env.SOURCE_VERSION ??
    "";
  return sha ? `${CACHE_VERSION}+${sha}` : CACHE_VERSION;
})();

/** The cache-version token stamped into saved reports + required on cache-read.
 *  Exported for tests (the /generate cache-read rejects reports whose embedded
 *  cacheVersion differs from this). */
export function getReportCacheVersion(): string {
  return REPORT_CACHE_VERSION;
}
...
export async function calculateDemandGaps(
  ...
  // Fix 2 — niche-aware demand windows. The evening (5-10pm) + weekend
  // "after-hours consumer demand" split is a CONSUMER / local-service assumption:
  // a homeowner with a burst pipe searches and calls after work. It is nonsense
  // for a B2B / non-trade business (a freight forwarder, a law firm) whose buyers
  // search during business hours. So we only assert after-hours / weekend missed
  // demand for real trade verticals where the 31%/31% split is defensible. For
  // general / non-trade businesses we return NO after-hours demand gaps (the
  // dollar/lead figures off them would be fabricated).
  const afterHoursDemandApplies = !isGeneralTrade(trade);
  ...
  const gaps: any[] = [];
  // Only build after-hours / weekend demand gaps for trade verticals where the
  // consumer after-hours search pattern is defensible (see afterHoursDemandApplies).
  if (afterHoursDemandApplies && !isOpenEvenings) {
  ...
  if (afterHoursDemandApplies && !isOpenWeekends) {
  ...
  return {
    ...
    isOpenEvenings,
    isOpenWeekends,
    // Whether the after-hours/weekend demand framing applies to this business
    // (true only for real trade verticals). Drives the narrative so non-trades
    // never get a contradictory "missing N after-hours enquiries" claim.
    afterHoursDemandApplies,
  };
}
...
/* ─── Broad / umbrella Google categories ──────────────────────────────────────
 * Some Google primaryTypes are a BROAD umbrella that, while not "generic" enough
 * to discard, are too imprecise to drive a competitor SEARCH well. The classic
 * case: a freight forwarder's primaryType is "shipping_service" / display
 * "Shipping Service", so searching "shipping service near <city>" surfaces
 * couriers + short-haul truckers, not actual freight forwarders. When layer 1
 * resolves to one of these, the cascade still runs the website/LLM layers to try
 * for a MORE specific sub-niche to drive the search query (the broad label stays
 * available for display). Keyed on both the snake_case type and the humanized
 * label so either signal matches. */
const BROAD_PLACE_TYPES = new Set<string>([
  ...
  "warehouse", "storage",
  "consultant", "corporate_office", "corporate office",
  "business_center", "business center",
  "marketing_agency", "marketing agency",
  "professional_services", "professional services",
]);

/** True when a resolved category label is a broad/umbrella Google category that
 *  is too imprecise to drive a precise competitor search (see BROAD_PLACE_TYPES). */
export function isBroadCategoryLabel(label?: string | null): boolean {
  const l = (label || "").toString().trim().toLowerCase();
  if (!l) return false;
  return BROAD_PLACE_TYPES.has(l) || BROAD_PLACE_TYPES.has(l.replace(/\s+/g, "_"));
}
...
export async function resolveCategoryLabelCascade(input: {
  ...
  chatFn?: (opts: any) => Promise<string>;
}): Promise<{ categoryLabel: string; searchCategory: string; layer: NicheLayer }> {
  ...
  // When layer 1 produced a BROAD/umbrella category ("Shipping Service"), it's a
  // fine DISPLAY label but a poor competitor-SEARCH term — it surfaces couriers
  // and short-haul truckers instead of actual freight forwarders. Keep it as the
  // display label, but fall through to the website/LLM layers to look for a MORE
  // specific sub-niche to drive the search. If those miss, the broad label is
  // used for search too (better than nothing).
  if (l1 && !isBroadCategoryLabel(l1)) {
    return { categoryLabel: l1, searchCategory: l1, layer: "primaryType" };
  }
  const broadDisplay = l1; // "" when layer 1 missed entirely; broad label otherwise
  ...
  if (l2 && !isBroadCategoryLabel(l2)) {
    // Specific sub-niche found → use it for SEARCH. Display: keep the broad
    // Google label when we had one (cleaner customer-facing wording), else the
    // sub-niche itself.
    return { categoryLabel: broadDisplay || l2, searchCategory: l2, layer: "website" };
  }

  // ── Layer 3: LLM classifier (reached when 1 missed/was broad AND 2 missed) ──
  ...
  if (l3 && !isBroadCategoryLabel(l3)) {
    return { categoryLabel: broadDisplay || l3, searchCategory: l3, layer: "llm" };
  }

  // ── No more-specific sub-niche found. If layer 1 gave a broad label, use it
  //    for both display AND search (better a broad real category than nothing). ──
  if (broadDisplay) {
    return { categoryLabel: broadDisplay, searchCategory: broadDisplay, layer: "primaryType" };
  }

  // ── Honest suppression — never fabricate, never "general". ──
  return { categoryLabel: "", searchCategory: "", layer: "none" };
}
...
export async function resolveTradeAndCategory(input: {
  ...
  chatFn?: (opts: any) => Promise<string>;
}): Promise<{ trade: string; categoryLabel: string; searchCategory: string; nicheLayer: NicheLayer }> {
  const types = Array.isArray(input.types) ? input.types : [];
  ...
  // For a real trade, display label AND search term are the trade word.
  let categoryLabel = trade;
  let searchCategory = trade;
  ...
  if (isGeneralTrade(trade)) {
    ...
    // searchCategory is the MOST SPECIFIC niche the cascade found (sub-niche when
    // the Google category was broad) — drives the competitor search + keyword seed.
    searchCategory = cascade.searchCategory;
    nicheLayer = cascade.layer;
  }
  return { trade, categoryLabel, searchCategory, nicheLayer };
}
...
export function buildTemplatedNarrative(ctx: {
  ...
  /** True only for trade verticals where the after-hours/weekend demand framing
   *  is defensible. For B2B / non-trade businesses this is false → the narrative
   *  must NOT assert after-hours/weekend missed demand. Defaults from isGeneralTrade. */
  afterHoursDemandApplies?: boolean;
  /** Whether any real demand gaps were actually detected (drives consistent copy
   *  — never claim "missing N" and "capturing well" in the same report). */
  hasDemandGaps?: boolean;
}): any {
  ...
  // After-hours framing is defensible only for trade verticals. For a B2B /
  // non-trade business (freight forwarder, law firm), buyers search in business
  // hours — so we never claim an evening/weekend coverage advantage or gap.
  const afterHoursApplies = ctx.afterHoursDemandApplies ?? !isGeneral;
  ...
  else if (afterHoursApplies && scores?.demandCoverage?.score >= 8) keyStrength = "Your availability (evenings/weekends) means you're positioned to capture demand competitors miss.";
  ...
  // Demand-gap insight (UI always wants a string here). Niche-aware + internally
  // consistent — Fix 2. We pick exactly ONE consistent message and never claim
  // "capturing well" AND "missing N" in the same breath:
  //   • Non-trade / B2B → no after-hours framing at all (it's a consumer pattern).
  //     Use a neutral business-hours message.
  //   • Trade, already covers evenings/weekends → "capturing after-hours demand"
  //     (no fabricated missed count).
  //   • Trade, NOT covering → frame the after-hours gap as the opportunity.
  const coversAfterHours =
    !!scores?.demandCoverage?.breakdown?.evenings || !!scores?.demandCoverage?.breakdown?.weekends;
  let demandGapInsight: string;
  if (!afterHoursApplies) {
    // B2B / non-trade: buyers search during business hours, so we make NO
    // evening/weekend/after-hours claim at all (that's a home-services consumer
    // assumption). Neutral message focused on visibility + responsiveness.
    demandGapInsight =
      "Your buyers tend to research during business hours, so the lever here is being easy to find and quick to respond when they search — not extended coverage.";
  } else if (coversAfterHours) {
    demandGapInsight = "Your evening/weekend availability means you're capturing after-hours demand many competitors miss.";
  } else {
    demandGapInsight = "Extending into evening/weekend coverage (even via call/chat handling) would capture demand you're currently missing.";
  }
...
router.post("/generate", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (business.placeId && !forceRefresh) {
      ...
      try {
        ...
        if (existing.length > 0) {
          ...
          // Cache-bust on deploy (Fix 3): only serve a cached report whose
          // embedded cacheVersion matches the CURRENT build's token. A report
          // generated by older code (or with no version stamp) fails this check
          // and we regenerate — so a deploy naturally invalidates stale reports.
          const cachedVersion = (cached.audit_data as any)?.cacheVersion;
          if (cachedVersion === REPORT_CACHE_VERSION) {
          ...
          log.info('[audit] cached report version stale — regenerating', {
            cachedVersion: cachedVersion ?? '(none)', currentVersion: REPORT_CACHE_VERSION,
          });
        }
      } catch (err) {
    ...
    const { trade, categoryLabel, searchCategory, nicheLayer } = await resolveTradeAndCategory({
    ...
    // The term we hand to the competitor SEARCH: a real trade, or — for a general
    // business — the MOST SPECIFIC sub-niche the cascade resolved (e.g. "freight
    // forwarding" rather than the broad "Shipping Service"), or "" (suppressed).
    // `categoryLabel` (possibly the broad Google label) is kept for DISPLAY.
    const competitorSearchCategory = isGeneralTrade(trade) ? searchCategory : trade;
    ...
    // For a non-trade business, seed keywords from the MOST SPECIFIC sub-niche
    // ("freight forwarding"), not the broad Google label ("Shipping Service") and
    // never the literal "general". searchCategory is "" for a truly-unknown
    // business → seeds are empty → table is suppressed.
    const seedKeywords = buildNicheKeywords(trade, city, businessNiche, business.name || '', searchCategory || categoryLabel);
    ...
    let compData = pick<any>(1);
    if (results[1]?.status === "rejected") log.warn("E1 competitors failed (degraded source):", { error: (results[1] as any).reason?.message });

    // ─── Precision-vs-coverage fallback (Fix 1) ───
    // We searched with the MOST SPECIFIC sub-niche (e.g. "freight forwarding").
    // If that precise search yielded too FEW real peers (<3) AND we have a
    // broader display label to fall back to (the broad Google category, e.g.
    // "Shipping Service"), broaden ONE notch deliberately rather than padding the
    // precise set with couriers. We prefer the precise result whenever it has
    // enough peers; we only broaden when precision starved us. Showing fewer,
    // more-relevant competitors is fine — we never collapse to zero if peers exist.
    const MIN_PRECISE_COMPETITORS = 3;
    const preciseCount = compData && Array.isArray(compData.competitors) ? compData.competitors.length : 0;
    const broadFallbackLabel = (categoryLabel || "").trim();
    const usedSpecificSearch =
      isGeneralTrade(trade) &&
      !!competitorSearchCategory &&
      !!broadFallbackLabel &&
      broadFallbackLabel.toLowerCase() !== competitorSearchCategory.toLowerCase() &&
      isBroadCategoryLabel(broadFallbackLabel);
    if (preciseCount < MIN_PRECISE_COMPETITORS && usedSpecificSearch) {
      log.info("[E1 competitors] specific niche under-covered — broadening one notch", {
        searchCategory: competitorSearchCategory, broadFallback: broadFallbackLabel, preciseCount,
      });
      try {
        const broadData = await fetchCompetitors(
          broadFallbackLabel, city, business.name, stateCode || undefined,
          getCached, setCached, bizCoords, business.placeId || undefined, reviewsCount,
        );
        const broadCount = broadData && Array.isArray(broadData.competitors) ? broadData.competitors.length : 0;
        if (broadCount > preciseCount) {
          log.info("[E1 competitors] broadened search yielded more peers — using it", {
            broadCount, preciseCount,
          });
          compData = broadData;
        } else {
          log.info("[E1 competitors] broadening did not help — keeping precise set", { broadCount, preciseCount });
        }
      } catch (err: any) {
        log.warn("[E1 competitors] broaden-one-notch fallback failed — keeping precise set:", { error: err?.message });
      }
    }
    ...
    const auditData: Record<string, any> = {
      // Cache-bust on deploy (Fix 3): stamp the current build's cache-version
      // token so the /generate cache-read can reject reports from older code.
...
2. NEVER suggest adding after-hours coverage if isOpenEvenings is true or if hours show operation past 9pm.
   2b. If afterHoursDemandApplies is false (a B2B / non-consumer business such as freight forwarding, logistics, professional services), NEVER claim the business is missing evening/weekend/after-hours demand and NEVER frame after-hours coverage as an opportunity — those buyers search during business hours. Do not state a missed-enquiry count tied to evenings/weekends. Frame demand around visibility and conversion instead.
   2c. NEVER write a contradiction: do not say the business is "capturing demand well" AND that it is "missing N enquiries" in the same report. Pick one consistent message from the data.
...
Rules for demandGapInsight: Always provide a string (never null). If afterHoursDemandApplies is false (B2B / non-trade), do NOT mention evenings/weekends/after-hours at all — write a neutral business-hours message focused on visibility and being easy to contact (e.g. "Your buyers search during business hours, so the lever here is visibility and fast response, not after-hours coverage."). If afterHoursDemandApplies is true and demand gaps exist, explain what they mean. If the business has full coverage (open 24hrs, evenings, weekends), say so positively — e.g. "Your 24/7 availability means you're capturing evening and weekend demand that competitors miss." Never combine a "capturing well" claim with a "missing N enquiries" claim.
...
afterHoursDemandApplies (evening/weekend demand framing is valid for this business type): ${demandData?.afterHoursDemandApplies ?? !isGeneralTrade(trade)}
...
${JSON.stringify(auditData, null, 2)}`;
        ...
        auditData.narrative = buildTemplatedNarrative({
          ...
          topMissingKeyword,
          afterHoursDemandApplies: demandData?.afterHoursDemandApplies ?? !isGeneralTrade(trade),
          hasDemandGaps: Array.isArray(auditData.demandGaps) && auditData.demandGaps.length > 0,
        });
