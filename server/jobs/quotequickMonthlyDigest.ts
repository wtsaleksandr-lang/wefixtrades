/**
 * QuoteQuick Monthly Digest Worker — Wave 75.
 *
 * Runs monthly (1st of each month at 09:15 UTC, staggered after
 * ContentFlow at 09:05 and TradeLine at 09:10). For every active enabled
 * quotequick client_service, compiles the past calendar month's data
 * and sends a recap email to the client.
 *
 * Per-client safety:
 *   - SQL-filters to active, enabled quotequick-* services with a
 *     non-empty contact_email
 *   - Idempotent per period via
 *     client_service.metadata.last_quotequick_digest_period
 *   - Honors clients.metadata.quotequick_notifications.monthly_digest_ready.email
 *     (default true)
 *   - Honors the global unsubscribe list inside the sender
 *   - Skips entirely when the client had no quotes this month
 *
...
import {
  sendQuotequickMonthlyDigest,
} from "../lib/quotequickMonthlyDigestEmail";
...
export async function processQuotequickMonthlyDigest(): Promise<DigestResult> {
  log.info("Starting QuoteQuick monthly digest sweep");
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const lastPeriod = typeof csMeta.last_quotequick_digest_period === "string"
        ? csMeta.last_quotequick_digest_period
        : "";
      const lastAt = typeof csMeta.last_quotequick_digest_at === "string"
        ? new Date(csMeta.last_quotequick_digest_at).getTime()
        : 0;
      ...
      const prefs = (clientMd.quotequick_notifications as Record<string, unknown>) ?? {};
      ...
      const sendResult = await sendQuotequickMonthlyDigest({
      ...
      if (sendResult.sent) {
        ...
        await db
          ...
          .set({
            metadata: {
              ...csMeta,
              last_quotequick_digest_at: new Date().toISOString(),
              last_quotequick_digest_period: label,
              last_quotequick_digest_quotes: sendResult.quotes_generated,
            },
            updated_at: new Date(),
          } as any)
        ...
        log.warn("QuoteQuick digest send skipped", {
          clientId: row.cs_client_id,
