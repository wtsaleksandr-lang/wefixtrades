...
- Main config files: `package.json`, `scripts/db/drizzle.config.dev.ts`, `vite.config.ts`, `tailwind.config.ts`
...
- `scripts/db/drizzle.config.dev.ts` — DB migration config (dev-only; moved out of repo root to hide from Replit's `drizzle.config*` deploy auto-push detector — the earlier rename from `drizzle.config.ts` to `drizzle.config.dev.ts` was not enough)
