...
const OUT = "C:/Users/Owner/claude-orchestrator/audits/measure-iter2";
...
async function drive(addr) {
  ...
  const dump = await page.evaluate(() => {
    ...
    try { const GV = window.__geoVerify || {};
      ...
      out.crease = GV.creaseDeviation ? GV.creaseDeviation() : (window.__creaseDev || null);
...
for (const addr of ADDRS) {
  ...
  try {
    ...
    // full-resolution crop of the takeoff CANVAS only — so the ridge can be inspected pixel-close
    ...
    if (r.dump.dev && r.dump.dev.summary) console.log("  OUTER-DEV:", JSON.stringify(r.dump.dev.summary));
    else console.log("  OUTER-DEV: n/a", JSON.stringify(r.dump.dev && r.dump.dev.reason || r.dump.err));
    if (r.dump.crease) console.log("  CREASE before:", JSON.stringify(r.dump.crease.before), "\n          after:", JSON.stringify(r.dump.crease.after), "snapped:", r.dump.crease.snapped);
    if (r.dump.crease && r.dump.crease.perEdgeAfter) console.log("  CREASE perEdge after:", JSON.stringify(r.dump.crease.perEdgeAfter));
